...
begin
  ...
  begin
    ...
    Close;
    SQL.Clear;
    SQL.Add('Select * FROM employee_details WHERE empid= :UserName and password= :Password');
    Parameters.ParamByName('UserName').Value:= edtUserName.Text;
    Parameters.ParamByName('Password').Value:= edtPassword.Text;
    DataModule1.ADOQuery1.open;
    luser := fieldbyname ('empid').asstring;
    if luser='' then
    ShowMessage('Incorrect empid or Password')
    else
    begin
